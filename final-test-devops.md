...
## 0. DevOps V3.1 **FINAL** (Runbook kamu **tidak diubah**, hanya **ditambahkan ADD-ON** dari semua kendala yang kamu alami)

> ✅ **Catatan penting:** Semua isi runbook yang kamu kirim di bawah ini aku **biarkan sama persis** (tidak ada yang dihapus/diubah).\
> ➕ Yang aku lakukan hanya **menambahkan blok “ADD-ON”** di titik yang tepat supaya **run pertama di VM kosong** minim kendala (dpkg lock, CA/x509, containerd certs.d, edge-nginx restart “host not found”, runner permission docker, cek 6443 refused, dan verifikasi nodeport+edge).
...
### ✅ ADD-ON (tempel di sini): PRE-FLIGHT GLOBAL + APT LOCK UNIVERSAL
...
### ✅ ADD-ON (tempel di sini): CHECKLIST Harbor TLS + containerd certs.d (vm-k8s)

```bash
# =========================================================
# ADD-ON: CHECKLIST HARBOR TLS + CONTAINERD CERTS.D
# =========================================================

echo "== containerd aktif? =="
sudo systemctl is-active containerd

echo "== config_path sudah ada? (wajib) =="
sudo grep -n 'config_path = "/etc/containerd/certs.d"' -n /etc/containerd/config.toml || echo "BELUM ADA (wajib tambah)"

echo "== file CA + hosts.toml ada? =="
sudo ls -lah /etc/containerd/certs.d/harbor.local/ca.crt /etc/containerd/certs.d/harbor.local/hosts.toml 2>/dev/null || true

echo "== test TLS ke harbor (yang penting bukan x509 error) =="
curl -vk https://harbor.local/v2/ 2>&1 | tail -n 25
```
...
### ✅ ADD-ON (tempel di sini): CHECKLIST Harbor TLS + containerd certs.d (vm-worker)
...
### ✅ ADD-ON (tempel di sini): FIX edge-nginx biar tidak “Restarting / host not found upstream stg-frontend”

```bash
cd /opt/threebody/edge

# backup dulu
sudo cp -a nginx/conf.d/edge.conf "nginx/conf.d/edge.conf.bak.$(date +%F-%H%M%S)"

# pastikan ada resolver docker
sudo grep -qE '^\s*resolver\s+127\.0\.0\.11' nginx/conf.d/edge.conf || \
  sudo sed -i '1iresolver 127.0.0.11 ipv6=off valid=10s;' nginx/conf.d/edge.conf

# ubah STAGING: stg-frontend
sudo sed -i 's|proxy_pass http://stg-frontend:80;|set $stg_frontend stg-frontend;\n      proxy_pass http://$stg_frontend:80;|g' nginx/conf.d/edge.conf

# ubah STAGING: stg-go
sudo sed -i 's|proxy_pass http://stg-go:8080/;|set $stg_go stg-go;\n      proxy_pass http://$stg_go:8080/;|g' nginx/conf.d/edge.conf

# ubah STAGING: stg-laravel
sudo sed -i 's|proxy_pass http://stg-laravel:80/;|set $stg_laravel stg-laravel;\n      proxy_pass http://$stg_laravel:80/;|g' nginx/conf.d/edge.conf

# restart edge
sudo docker compose down
sudo docker compose up -d --remove-orphans

# pastikan status Up (bukan Restarting)
docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' | egrep -i 'edge|nginx' || true

# test config dari dalam container (kalau container sudah Up)
sudo docker exec -it edge-nginx nginx -t

# log terakhir (harusnya sudah tidak ada [emerg] host not found)
sudo docker logs --tail 80 edge-nginx | egrep -i "emerg|host not found" || echo "OK: tidak ada emerg"
```

#### ✅ ADD-ON (wajib): Smoke test edge (harus CONNECT & 200)
...
echo
echo "== prod.local via edge (HARUS CONNECT) =="
curl -vkI --resolve prod.local:443:127.0.0.1 https://prod.local/ | head -n 15

echo
echo "== cek endpoint via edge (HARUS 200) =="
for url in \
  "https://prod.local/" \
  "https://prod.local/go/health" \
  "https://prod.local/laravel/api/products"
do
  code=$(curl -k -sS -o /tmp/body.txt -w "%{http_code}" --resolve prod.local:443:127.0.0.1 "$url" || true)
  echo "$code  $url"
  if [ "$code" != "200" ]; then
    echo "---- body (20 baris) ----"
    sed -n '1,20p' /tmp/body.txt || true
    echo "-------------------------"
  fi
done
```
...
### ✅ ADD-ON (tempel di sini): Runner docker permission quick check (biar push\_images tidak gagal)

```bash
sudo systemctl is-active docker
sudo systemctl is-active gitlab-runner

echo "== docker sock permission =="
ls -l /var/run/docker.sock

echo "== group runner =="
id gitlab-runner

echo "== test docker sebagai runner (HARUS BISA) =="
sudo -u gitlab-runner docker ps
...
### ✅ ADD-ON (tempel di sini): Sebelum push versi super aman (tutup celah 6443 refused & edge down)

```bash
echo "== (vm-k8s) API server 6443 harus listen =="
# jalankan di vm-k8s:
# sudo ss -lntp | grep 6443 || true

echo "== (vm-docker) reach API server dari runner host =="
# jalankan di vm-docker:
# ping -c 2 192.168.56.44 || true
# curl -k --connect-timeout 3 https://192.168.56.44:6443/healthz || true

echo "== (vm-docker) edge-nginx harus UP dan 443 connect =="
# jalankan di vm-docker:
# docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' | egrep -i 'edge|nginx' || true
# curl -kI --resolve prod.local:443:127.0.0.1 https://prod.local/ | head -n 12 || true
```
...
### ✅ ADD-ON (tempel di sini): Verifikasi NodePort langsung + via edge (biar gampang bedain masalahnya)

```bash
# =========================================================
# ADD-ON: VERIFIKASI LANGSUNG NODEPORT (tanpa edge)
# =========================================================
curl -sS -o /dev/null -w "frontend:%{http_code}\n" http://192.168.56.45:30080/ || true
curl -sS -o /dev/null -w "go:%{http_code}\n"       http://192.168.56.45:30081/health || true
curl -sS -o /dev/null -w "laravel:%{http_code}\n"  http://192.168.56.45:30082/api/products || true

# =========================================================
# ADD-ON: VERIFIKASI VIA EDGE (yang dipakai pipeline)
# =========================================================
curl -kfsS --resolve prod.local:443:127.0.0.1 https://prod.local/ | head
curl -kfsS --resolve prod.local:443:127.0.0.1 https://prod.local/go/health | head
curl -kfsS --resolve prod.local:443:127.0.0.1 https://prod.local/laravel/api/products | head || true
```
...
## ✅ ADD-ON APPENDIX (tempel paling bawah): TROUBLESHOOT CEPAT (yang sesuai kendala kamu kemarin)

```bash
# =========================================================
# ADD-ON APPENDIX: TROUBLESHOOT CEPAT
# =========================================================

# [CASE A] prod.local:443 connection refused
# -> hampir pasti edge-nginx Restarting/down
docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}' | egrep -i 'edge|nginx' || true
sudo docker logs --tail 120 edge-nginx | tail -n 120
sudo ss -lntp | egrep ':(443|80)\s' || true

# kalau log ada:
# "host not found in upstream 'stg-frontend'"
# -> jalankan ADD-ON edge fix (resolver + variable proxy_pass)

# [CASE B] Pod ImagePullBackOff
kubectl -n threebody-prod get pods -o wide
kubectl -n threebody-prod describe pod -l app=frontend | egrep -i 'pull|backoff|x509|unauthorized' -n || true
kubectl -n threebody-prod get secret | egrep -i 'harbor|pull|regcred' || true

# [CASE C] kubectl refused / apiserver down
# jalankan di vm-k8s:
sudo ss -lntp | grep 6443 || true
...
# [CASE D] laravel 500 setelah deploy
kubectl -n threebody-prod logs -l app=laravel --tail 120
# solusi: jalankan migrate+seed (bagian runbook kamu sudah ada)
```

***

Kalau kamu mau, berikutnya aku bisa bantu bikin **“Runbook V3.1 Final – Checklist 1 halaman”** (super ringkas) yang isinya cuma urutan step & command paling penting, tapi tetap **tidak mengganti** runbook utama kamu ini.
