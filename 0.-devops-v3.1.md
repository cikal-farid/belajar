...
# pastikan config_path aktif
sudo awk '
{print}
$0 ~ /\[plugins\."io\.containerd\.grpc\.v1\.cri"\.registry\]/ && !seen {
  seen=1
}
' /etc/containerd/config.toml >/dev/null

# set config_path (aman walau sudah ada)
...
### 7. Trust CA Harbor untuk containerd
...
# ganti USERNAME kamu:
git remote add gitlab git@gitlab-443:USERNAME/three-body-problem-main.git
